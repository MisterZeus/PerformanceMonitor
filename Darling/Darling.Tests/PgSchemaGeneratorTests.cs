...
public sealed class PgSchemaGeneratorTests
{
    ...
    public void Catalog_CoversAllCollectors_WithUniqueTablesAndNames()
    {
        /* 35 through agent_status + long_query_completions (#1496 long-query trace) = 36, plus the two
           ...
           collector) = 42. The catalog is deliberately engine-mixed: the schema generator walks it to
        ...
        Assert.Equal(42, CollectorCatalog.All.Count);
        Assert.Equal(42, CollectorCatalog.All.Select(s => s.TargetTable).Distinct().Count());
        Assert.Equal(42, CollectorCatalog.All.Select(s => s.Name).Distinct().Count());
    }
