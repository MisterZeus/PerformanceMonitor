...
public sealed class ViewerCollectorCoverageTests
{
    ...
    private static readonly HashSet<string> KnownStoreOnlyOrUnbuiltTables = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        // The seven PostgreSQL collector tables are read through MCP today, not the WPF viewer: each has a
        // reader class + MCP tool (get_pg_wait_stats, get_pg_top_queries, get_pg_wraparound_risk,
        // get_pg_xmin_horizon, get_pg_replication_slots, get_pg_autovacuum_health, get_pg_io_stats) and is
        // exposed over /api/read, so the data is not invisible — it is just not on a WPF tab, because the
        // viewer's surfaces are SQL-Server-shaped and a PostgreSQL target has no CPU/wait/blocking analogue
        // to slot into. Deliberately listed one per line so removing one at a time is a one-line diff.
        "pg_wait_stats",
        "pg_statement_stats",
        "pg_wraparound_stats",
        "pg_xmin_horizon",
