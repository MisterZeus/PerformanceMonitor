...
At startup the viewer writes **which of those rules won**, the absolute path it produced, and whether that file exists to `%APPDATA%\PerformanceMonitorDarling\logs\darling-viewer_yyyyMMdd.log` — before it tries to read the file, so a missing or malformed one still says where it looked. Once the file loads it adds a non-secret summary of what it parsed (host, port, username, database, SSL mode, search path, whether the connection string was read verbatim or derived from `postgres.managed`, and the certificate path plus whether that file exists). Credentials are never written. The same block appears in the connection-failure window with a **Copy details** button — see [Troubleshooting](#troubleshooting).
...
**"Root Certificate ... exists: NO"** — with `SSL Mode=VerifyFull`, a **relative** `Root Certificate` path resolves against the viewer's **working directory**, so a shortcut with a different "Start in" folder and a shell launch look in different places. The diagnostics block prints the absolute path it actually opened; either put `server.crt` there or make the `Root Certificate` value an absolute path. Re-run `--print-viewer-connection` on the store host if you no longer have the certificate — it regenerates if the bind IP changes, so an old copy stops matching.
...
Copy the three files to the viewer machine and put them **next to the Viewer executable** — that works with nothing edited. To keep them somewhere else instead, point `DARLING_CONFIG` at the exported `darling.json` **and** change its `Root Certificate` to the full path of the `server.crt` beside it: a bare file name follows the Viewer's *working directory*, not the config's folder, so that one edit is what makes an out-of-the-way folder work ([issue #1970](https://github.com/erikdarlingdata/PerformanceMonitor/issues/1970) tracks removing the condition viewer-side).
