...
public sealed class DarlingMcpPgWaitTools
{
        ...
        [Description(McpHelpers.AsOfDescription)] string? as_of = null)
    {
        ...
        try
        {
            ...
            if (rows.Count == 0)
            {
                /* Both halves of that ambiguity are answerable when the store knows the engine (#2532): a
                   SQL Server target gets the dialect answer, a stock PostgreSQL one gets the Aurora-only
                   answer, and both say the gap is permanent instead of inviting a hunt. The sentence below
                   stays for the row whose engine_kind is NULL, where no claim can be made. */
