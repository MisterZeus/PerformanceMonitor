...
internal static class DarlingMcpInstructions
{
    ...
    private const string Body = """
        ...
        - `analyze_server`, `get_analysis_facts`, `compare_analysis`, `get_analysis_findings`, `get_pvs_stats` and `get_fleet_overview` do not take it either. The first four window inside the analysis engine rather than at the read; the last two mix a latest-snapshot measurement with a windowed one, so anchoring only the windowed half would return a result whose two halves describe different instants.
