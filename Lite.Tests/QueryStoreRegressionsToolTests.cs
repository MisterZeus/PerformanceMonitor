...
public sealed class QueryStoreRegressionsToolTests : IClassFixture<SharedDuckDbFixture>, IDisposable
{
    ...
    public async Task FourKindsOfNothing_AreFourDifferentAnswers()
    {
        ...
        /*
            2. recent rows but NO baseline - the branch this read exists to get right. Everything sits
            inside the window, so no regression is detectable however badly one regressed, and answering
            "no regressions" would be a confident wrong answer rather than a missing one.
        */
        await SeedAsync(baseNow.AddMinutes(-30), executions: 100, avgDurationUs: 1000, avgCpuUs: 1000, intervalId: 1);

        var noBaseline = Root(await McpQueryTools.GetQueryStoreRegressions(service, _serverManager, ServerName, 24));
