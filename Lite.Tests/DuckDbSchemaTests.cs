...
public class DuckDbSchemaTests : IDisposable
{
    ...
    public void SchemaStatements_MatchTableCount()
    {
        ...
        /* 47 tables from Schema (schema_version is created separately by DuckDbInitializer).
           ...
           (#1962 per-server state a collector's own rows cannot produce), and pvs_stats
           (#1951 ADR persistent version store). */
